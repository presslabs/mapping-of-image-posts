...
add_filter( 'plugin_action_links_' . plugin_basename( __FILE__ ), 'mapping_of_image_posts_settings_link' );
function mapping_of_image_posts_settings_link( $links ) {
	$settings_link = '<a href="tools.php?page=' . plugin_basename( __FILE__ ) . '&tab=settings">Settings</a>';
...
function mapping_of_image_posts_rand_letter() {
	$text = str_shuffle( 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ' );
	return $text[0];
}
...
function mapping_of_image_posts_update_options() {
	...
	<?php } else { ?>
		...
		<p><strong><span style="color:brown;">Error saving options! (<?php echo $error; ?>) </span></strong></p>
		</div>
	<?php }
}
...
<div class="postbox" style="float:left; display:block; width:auto; height:auto; padding:10px;margin-top:10px;">
	...
	<div id='messages'><div class="row-title">Ready to scan!</div></div>
...
<?php if ( 'settings' === $tab ) {
	...
	for ( $key = 0; $key < 4; $key++ ) {
		if ( '1' == $moip_image_type[ $key ] ) {
			$image_type[ $key ] = ' checked="checked"';
		}
	}
	...
	if ( 'nginx' === $moip_out_file_type ) {
		$file_type[0] = ' checked="checked"';
	}

	if ( 'apache' === $moip_out_file_type ) {
		$file_type[1] = ' checked="checked"';
	}
?>
<form method="post">
...
function mapping_of_image_posts_menu() {
	add_management_page( 'Mapping - Options', 'Mapping', 'administrator', __FILE__, 'mapping_of_image_posts_options' );
}
